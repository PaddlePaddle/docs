...
输入:
    - **inputs** (Tensor) - 输入（可以是多层嵌套的）。如果time_major为True，则Tensor的形状为[batch_size,time_steps,input_size]，如果time_major为False，则Tensor的形状为[time_steps,batch_size,input_size]，input_size为cell的input_size。
...
输出:
    - **outputs** (Tensor|list|tuple) - 输出。如果time_major为True，则Tensor的形状为[batch_size,time_steps,hidden_size]，如果time_major为False，则Tensor的形状为[time_steps,batch_size,hidden_size]。
