...
.. py:function:: paddle.vision.ops.yolo_loss(x, gt_box, gt_label, anchors, anchor_mask, class_num, ignore_thresh, downsample_ratio, gt_score=None, use_label_smooth=True)
...
返回类型:   变量（Tensor），数据类型与input一致。

抛出异常:
    - ``TypeError``  – yolov3_loss的输入x必须是Tensor
    - ``TypeError``  – 输入yolov3_loss的gtbox必须是Tensor
    - ``TypeError``  – 输入yolov3_loss的gtlabel必须是None或Tensor
    - ``TypeError``  – 输入yolov3_loss的gtscore必须是Tensor
