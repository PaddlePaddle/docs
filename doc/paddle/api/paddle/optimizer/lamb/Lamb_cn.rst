...
参数：
  ...
  - **grad_clip** (GradientClipBase, 可选) – 梯度裁剪的策略，支持三种裁剪策略： :ref:`cn_api_paddle_fluid_clip_ClipGradByGlobalNorm` 、 :ref:`cn_api_paddle_fluid_clip_ClipGradByNorm` 、 :ref:`cn_api_paddle_fluid_clip_ClipGradByVelur` 。默认值为None，此时将不进行梯度裁剪。
