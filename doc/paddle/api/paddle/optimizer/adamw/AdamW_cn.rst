...
参数: 
    ...
    - **apply_decay_param_fun** (function|None, 可选): 传入函数时，只有可以使 apply_decay_param_fun(Tensor)==True的Tensor会进行weight decay更新。只有在想要指定特定需要进行weight decay更新的参数时使用。默认值为None
    - **grad_clip** (GradientClipBase, 可选) – 梯度裁剪的策略，支持三种裁剪策略： :ref:`GradientClipByGlobalNorm <cn_api_fluid_clip_GradientClipByGlobalNorm>` 、 :ref:`GradientClipByNorm <cn_api_fluid_clip_GradientClipByNorm>` 、 :ref:`GradientClipByValue <cn_api_fluid_clip_GradientClipByValue>` 。
      默认值为None，此时将不进行梯度裁剪。
