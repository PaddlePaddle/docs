...
----------
##### 4.问题：如何将本地数据传入`fluid.dygraph.Embedding`的参数矩阵中？
...
##### 5.问题：如何实现网络层中多个feature间共享该层的向量权重？
...
----------
##### 6.问题：如何修改全连接层参数，如：weights、bias、optimizer.SGD？
...
----------
##### 9.问题：load_inference_model在加载预测模型时能否用py_reader读取？

+ 答复：目前`load_inference_model`加载进行的模型还不支持py_reader输入。

----------
##### 10.问题：使用`py_reader`读取数据时，如何给变量命名？
...
----------
##### 12.问题：使用NVIDIA多卡运行Paddle时报错，`Error：NCCL ContextMap`或者`Error：hang住`（log日志打印突然卡住），如何解决？
...
----------
##### 13.问题：使用optimizer或ParamAttr设置的正则化和学习率，二者什么差异？
...
----------
##### 15.问题：如何导出指定层的权重，如导出最后一层的*weights*和*bias*？
...
----------
##### 16.问题：使用X2paddle 从Caffe 转Paddle model时，报错 `TypeError: __new__() got an unexpected keyword argument 'serialized_options'` ，如何处理？
...
----------
##### 18.问题：使用多卡或多GPU进行数据并行时，如何设置异步数据读取？
...
----------
##### 19.问题：如何从np.array生成一个具有Shape和DType的Tensor?
...
----------
##### 21.问题：使用CPU或GPU时，如何设置`num_threds`？
...
----------
##### 24.问题：Executor与ParallelExecutor有什么区别？
...
----------
##### 31.问题：使用`paddle.dataset.mnist.train()`获得数据后，如何转换为可操作的Tensor？

+ 答复：执行`fluid.dygraph.to_varibale()`，将data数据转化为可以操作的动态图Tensor。

----------
##### 32.问题：如何给图片添加一个通道数，并进行训练？
...
----------
##### 34.问题：在AI Studio上使用GPU训练时报错 `Attempt to use GPU for prediction, but environment variable CUDA_VISIBLE_DEVICES was not set correctly.`，如何处理？

+ 答复：需要在Notebook环境中增加：`%set_env CUDA_VISIBLE_DEVICES=0`。

----------

##### 35.问题：优化器设置时报错`AttributeError: parameter_list argument given to the Optimizer should not be None in dygraph mode.`，如何处理？
...
----------
##### 36.问题：`fluid.layer.pool2d`的全局池化参数和设置参数有关系么？

+ 答复：如果设置`global_pooling`，则设置的`pool_size`将忽略，不会产生影响。

----------
##### 37.问题：多卡训练时启动失败，`Error：Out of all 4 Trainers`，如何处理？
...
----------
##### 38.问题：GPU内存不足，报错 `Error：Out of memory error GPU`，如何处理？
...
----------
##### 39.问题：使用GPU训练时报错，`Error：incompatible constructor arguments.`，如何处理？
...
----------
##### 40.问题：`paddle.fluid.layers.py_reader`和`fluid.io.PyReader`有什么区别？

+ 答复：两个都是异步的。推荐使用`fluid.io.PyReader`。

----------
##### 41.问题：请问`fluid.layers.matmul`和`fluid.layers.mul`有什么区别？

+ 答复：`matmul`支持broadcast和任意阶相乘。`mul`会把输入都转成两维去做矩阵乘。

----------
##### 42.问题：如何初始化一个随机数的Tensor？
