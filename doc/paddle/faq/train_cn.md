...
**如果您在使用飞桨时遇到问题，请先查阅FAQ；如果下面的FAQ无法无法解决，请ISSUE提问，谢谢！**
...
----------
**5.问题**：如何修改全连接层参数，如：weights、bias、optimizer.SGD？
...
----------
**8.问题**：load_inference_model在加载预测模型时能否用py_reader读取？

**答复**：目前`load_inference_model`加载进行的模型还不支持py_reader输入。

----------
**9.问题**：使用`py_reader`读取数据时，如何给变量命名？
...
----------
**11.问题**：使用NVIDIA多卡运行Paddle时报错，`Error：NCCL ContextMap`或者`Error：hang住`（log日志打印突然卡住），如何解决？
...
----------
**12.问题**：使用optimizer或ParamAttr设置的正则化和学习率，二者什么差异？
...
----------
**14.问题**：如何导出指定层的权重，如导出最后一层的*weights*和*bias*？
...
----------
**15.问题**：使用X2paddle 从Caffe 转Paddle model时，报错 `TypeError: __new__() got an unexpected keyword argument 'serialized_options'` ，如何处理？

**答复**：这是由于ProtoBuf版本较低导致，将protobuf升级到3.6.0即可解决。

----------
**16.问题**：使用多卡或多GPU进行数据并行时，如何设置异步数据读取？
...
----------
**17.问题**：如何从np.array生成一个具有Shape和DType的Tensor?
...
----------
**19.问题**：使用CPU或GPU时，如何设置`num_threds`？
...
----------
**22.问题**：Executor与ParallelExecutor有什么区别？

**答复**：如果没有指定Scope，所有的Executor都会共享一个Scope，即`global_scope`。

- `fluid.Executor`执行对象是Program，可以认为是一个轻量级的执行器，目前主要用于参数初始化、参数加载、参数模型保存。
- `fluid.ParallelExecutor`的执行对象是Graph，ParallelExecutor内部会将Program转为Graph，这样更便于对模型进行分析。

----------
**23.问题**：import paddle.fluid后logging模块无法使用，如何解决？ 

**答复**：操作方法可以参考[#issue17731](https://github.com/PaddlePaddle/Paddle/issues/17731)。

----------
**24.问题**：Windows环境下，出现"Windows not support stack backtrace yet"，如何处理？

**答复**：Windows环境下，遇到程序报错不会详细跟踪内存报错内容。这些信息对底层开发者更有帮助，普通开发者不必关心这类警告。如果想得到完整内存追踪错误信息，可以尝试更换至Linux系统。

----------
**25.问题**：训练过程中提示显存不足，如何处理？
...
----------
**29.问题**：使用`paddle.dataset.mnist.train()`获得数据后，如何转换为可操作的Tensor？

**答复**：执行`fluid.dygraph.to_varibale()`，将data数据转化为可以操作的动态图Tensor。

----------
**30.问题**：如何给图片添加一个通道数，并进行训练？
...
----------
**32.问题**：在AI Studio上使用GPU训练时报错 `Attempt to use GPU for prediction, but environment variable CUDA_VISIBLE_DEVICES was not set correctly.`，如何处理？

**答复**：需要在Notebook环境中增加：`%set_env CUDA_VISIBLE_DEVICES=0`。

----------
**33.问题**：优化器设置时报错`AttributeError: parameter_list argument given to the Optimizer should not be None in dygraph mode.`，如何处理？
...
----------
**34.问题**：`fluid.layer.pool2d`的全局池化参数和设置参数有关系么？

**答复**：如果设置`global_pooling`，则设置的`pool_size`将忽略，不会产生影响。

----------
**35.问题**：多卡训练时启动失败，`Error：Out of all 4 Trainers`，如何处理？
...
----------
**36.问题**：GPU内存不足，报错 `Error：Out of memory error GPU`，如何处理？
...
----------
**37.问题**：使用GPU训练时报错，`Error：incompatible constructor arguments.`，如何处理？
...
----------
**38.问题**：`paddle.fluid.layers.py_reader`和`fluid.io.PyReader`有什么区别？

**答复**：两个都是异步的。推荐使用`fluid.io.PyReader`。

----------
**39.问题**：请问`fluid.layers.matmul`和`fluid.layers.mul`有什么区别？

**答复**：`matmul`支持broadcast和任意阶相乘。`mul`会把输入都转成两维去做矩阵乘。

----------
**40.问题**：如何初始化一个随机数的Tensor？
