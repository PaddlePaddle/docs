...
The parameters in ``emb`` layer and ``fc_none`` , ``fc_none1`` are named by the ``OPName_number`` , such as ``embedding_0.w_0`` 、 ``fc_0.w_0`` 、 ``fc_0.b_0`` . And the parameter prefix is consistent with the network layer prefix. ``my_fc1`` and ``my_fc2`` use the specified :code:`name` as prefix to name the parameters, where the number in ``my_fc.b_0`` and ``my_fc.b_1`` is automatically incremented to distinguish the bias parameter under the same :code:`name` . ``fc_weight`` is the shared weight name.
