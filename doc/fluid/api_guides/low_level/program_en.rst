...
In Fluid, some operators contain the parameter :code:`name` , such as :ref:`api_fluid_layers_fc` . This :code:`name` is generally used as the prefix identification of operator output and weight. The specific rules are as follows:

* Prefix identification for operator output. If :code:`name` is specified in the operator, Fluid will name the operator output with ``nameValue_number.tmp_number`` . If the :code:`name` is not specified, ``OPName_number.tmp_number`` is automatically generated to name the layer. The numbers are automatically incremented to distinguish different network layers under the same operator. 
