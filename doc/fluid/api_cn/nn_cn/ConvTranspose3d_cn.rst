...
如果指定了output_size， 该算子可以自动计算滤波器的大小。

参数:
  - **in_channels** (int) - 输入图像的通道数。
  - **out_channels** (int) - 滤波器的个数，和输出特征图个数相同。
  - **kernel_size** (int|list|tuple) - 滤波器大小。如果kernel_size是一个元组，则必须包含三个整型数，（kernel_size_depth，kernel_size_height, kernel_size_width）。否则，kernel_size_depth = kernel_size_height = kernel_size_width = kernel_size。如果kernel_size=None，则必须指定output_size， 该算子内部会根据output_size、padding和stride计算出滤波器大小。默认：None。output_size和kernel_size不能同时为None。
