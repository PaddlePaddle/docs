...
.. code-block:: python
    ...
    data_1 = paddle.data(name='data_1', shape=[2, 4, 6], dtype='float32')
    reshaped_1 = paddle.reshape(x=data_1, shape=[-1, 0, 3, 2], inplace=True)
    data_2 = paddle.full([2, 25], 'int32', 3, device=None, stop_gradient=True)
    dim = paddle.full([1], 'int32', 5, device=None, stop_gradient=True)
    reshaped_2 = paddle.reshape(data_2, shape=[dim, 10])
    # the shape of reshaped_2 is [5,10].
    
    # example 3:
    data_3 = paddle.data(name='data_3', shape=[2, 4, 6], dtype='float32')
    reshaped_3 = paddle.reshape(x=data_3, shape=[6, 8])
