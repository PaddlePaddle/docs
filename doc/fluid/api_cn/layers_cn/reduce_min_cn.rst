...
默认计算所有维度的最小值，当给定dim的值得时候计算给定维度上张量（Tensor）元素最小值。

参数：
          - **input** （Variable）：输入变量为Tensor或LoDTensor。
          - **dim** （list | int | None）：函数运算的维度。如果为None，则对输入的所有元素求最小值并返回单个元素的Tensor变量，dim维度的大小必须在  :math:`[−rank(input),rank(input))` 范围内。如果 :math:`dim [i] <0` ，则实际维度的值为 :math:`rank+dim[i]` 。
...
返回：  在某些特定维度或者所有维度计算最小值之后的Tensor变量结果。
