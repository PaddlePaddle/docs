...
..  code-block:: python
    ...
    paddle.min(x, out=None)
    paddle.min(x, dim=0, out=None)
    paddle.min(x, dim=-1, out=None)
    paddle.min(x, dim=1, keep_dim=True, out=None)
    y = paddle.data(name='y', shape=[2, 2, 2], dtype='float32')
    paddle.min(y, dim=[1, 2], out=None)
    paddle.min(y, dim=[0, 1], out=None)
