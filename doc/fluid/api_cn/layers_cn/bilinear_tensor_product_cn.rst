...
在这个公式中：
  - :math:`x`: 第一个输入，包含 :math:`M` 个元素，形状为 :math:`[batch_size, M]` 。
  - :math:`y`: 第二个输入，包含 :math:`N` 个元素，形状为 :math:`[batch_size, N]` 。
  - :math:`W_{i}`: 第 :math:`i` 个被学习的权重，形状是 :math:`[M, N]` 。
  - :math:`out_{i}`: 输出的第 :math:`i` 个元素，形状是 :math:`[batch_size, size]` 。
  - :math:`y^\mathrm{T}`: :math:`y_{2}` 的转置。

参数：
    - **x** (Variable): 2-D 输入张量，形状为 :math:`[batch_size, M]` 。
    - **y** (Variable): 2-D 输入张量，形状为 :math:`[batch_size, N]` 。
    - **size** (int): 此层的维度。
