...
..  code-block:: python
    ...
    beam_size = 4
    end_id = 1
    pre_ids = fluid.layers.data(name='pre_id', shape=[1], lod_level=2, dtype=
        'int64')
    pre_scores = fluid.layers.data(name='pre_scores', shape=[1], lod_level=2,
        dtype='float32')
    probs = fluid.layers.data(name='probs', shape=[10000], dtype='float32')
    topk_scores, topk_indices = paddle.topk(probs, k=beam_size)
    accu_scores = paddle.add(x=paddle.log(x=topk_scores), y=paddle.reshape(
        pre_scores, shape=[-1]), alpha=1, out=None)
    selected_ids, selected_scores = paddle.nn.beam_search(pre_ids=pre_ids,
        pre_scores=pre_scores, ids=topk_indices, scores=accu_scores, beam_size=
        beam_size, end_id=end_id)
