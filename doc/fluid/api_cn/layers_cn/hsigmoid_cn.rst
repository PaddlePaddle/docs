...
..  code-block:: python
    ...
    x = paddle.full(shape=[4, 3], fill_value=0.9, dtype='float32', device=None,
    # x = [[0.9, 0.9, 0.9], [0.9, 0.9, 0.9], [0.9, 0.9, 0.9], [0.9, 0.9, 0.9]]
        stop_gradient=True)
    y = paddle.full(shape=[4, 1], fill_value=1, dtype='int64', device=None,
        stop_gradient=True)
    out = paddle.nn.functional.hsigmoid(input=x, label=y, num_classes=2,
        param_attr=paddle.nn.initializer.Constant(value=0.05), bias_attr=paddle
        .nn.initializer.Constant(value=0.0))
