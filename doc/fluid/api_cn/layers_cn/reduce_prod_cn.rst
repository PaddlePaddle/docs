...
..  code-block:: python
    ...
    paddle.reduce_prod(x)
    paddle.reduce_prod(x, dim=0)
    paddle.reduce_prod(x, dim=-1)
    paddle.reduce_prod(x, dim=1, keep_dim=True)
    y = paddle.data(name='y', shape=[2, 2, 2], dtype='float32')
    paddle.reduce_prod(y, dim=[1, 2])
    paddle.reduce_prod(y, dim=[0, 1])
