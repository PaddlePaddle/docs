...
**此OP仅支持LoDTensor** ，将对输入的LoDTensor进行reshape，以输出包含新维度（new_dim）下的LoDTensor。 在指定 ``new_dim`` 参数下，通过序列原始长度、和张量的原始shape计算出新的shape。目前仅支持1-level LoDTensor，请确保(原长度*原维数)可以除以新的维数，且每个序列没有余数。
...
参数：
    - **input** (Variable) - 维度为 :math:`[M, K]`的LoDTensor，且仅支持lod_level为1。
