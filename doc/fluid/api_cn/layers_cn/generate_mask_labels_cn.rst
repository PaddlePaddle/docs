...
.. code-block:: python
    ...
    im_info = paddle.data(name='im_info', shape=[None, 3], dtype='float32')
    gt_classes = paddle.data(name='gt_classes', shape=[None, 1], dtype=
        'float32', lod_level=1)
    is_crowd = paddle.data(name='is_crowd', shape=[None, 1], dtype='float32',
        lod_level=1)
    gt_masks = paddle.data(name='gt_masks', shape=[None, 2], dtype='float32',
        lod_level=3)
    rois = paddle.data(name='rois', shape=[None, 4], dtype='float32', lod_level=1)
    roi_labels = paddle.data(name='roi_labels', shape=[None, 1], dtype='int32',
        lod_level=1)
    mask_rois, mask_index, mask_int32 = fluid.layers.generate_mask_labels(im_info
        =im_info, gt_classes=gt_classes, is_crowd=is_crowd, gt_segms=gt_masks,
        rois=rois, labels_int32=roi_labels, num_classes=81, resolution=14)
