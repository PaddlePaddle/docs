...
..  code-block:: python
    ...
    paddle.max(x, out=None)
    paddle.max(x, dim=0, out=None)
    paddle.max(x, dim=-1, out=None)
    paddle.max(x, dim=1, keep_dim=True, out=None)
    y = paddle.data(name='y', shape=[2, 2, 2], dtype='float32')
    paddle.max(y, dim=[1, 2], out=None)
    paddle.max(y, dim=[0, 1], out=None)
