...
参数：
    - **input** (Tensor) - 数据类型为float，double的Tensor。输入特征图，格式为NCHW。
    ...
    - **name** (str|None) - 先验框操作符名称。默认：None

返回：
    - Variable(Tensor): 数据类型为float，double的Tensor。输出anchor，格式为[H,W,num_anchors,4]。 ``H``  是输入的高度， ``W`` 是输入的宽度， ``num_anchors`` 是输入每位的框数,每个anchor格式（未归一化）为(xmin,ymin,xmax,ymax)

    - Variable(Tensor): 数据类型为float，double的Tensor。anchor的variances, 格式为 [H,W,num_priors,4]。 ``H`` 是输入的高度， ``W`` 是输入的宽度， ``num_priors`` 是输入每个位置的框数,每个变量的格式为(xcenter,ycenter,w,h)。
