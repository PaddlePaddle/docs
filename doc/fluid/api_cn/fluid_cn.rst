...
.. note::
     1. 如果feed参数为dict类型，那么传入 ``ParallelExecutor`` 的数据量 *必须* 大于可用的执行场所数目（即CPU或GPU的卡数）。否则，C++端将会抛出异常。应额外注意核对数据集的最后一个batch是否比可用执行场所数目（即CPU或GPU的卡数）大。
     2. 如果可用执行场所（即CPU或GPU的卡数）大于一个，则为每个变量最后获取的结果都是list类型，且这个list中的每个元素都是各个可用执行场所（即CPU或GPU）的变量
