...
注意：
   - 有些变量不是参数，如学习率，全局训练步数（global step）等，但它们对于训练却是必要的。因此，调用 :ref:`cn_api_fluid_io_save_params()` 和 `cn_api_fluid_io_load_params()` 来保存和加载参数对于断点训练是不够的，这种情况下可以使用 :ref:`cn_api_fluid_io_save_persistables()` 和 `cn_api_fluid_io_load_persistables()` 来保存和加载训练过程中的检查点（checkpoint）。
