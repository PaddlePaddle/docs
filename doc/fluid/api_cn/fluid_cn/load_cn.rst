...
当save_params、save_persistables或save_vars保存的模型格式为单个大文件时，var_list不能为None。
...
参数:
 ...
 - **var_list** (list, 可选) - 需要加载的变量列表（仅当加载旧接口[save_params，save_persistables，save_vars]接口保存的模型），如果之前存储了多个小文件，变量列表可以是保存变量的子集，如果是单个大文件，变量列表必须和保存的变量完全一致。
