...
##### 问题：如何将本地数据传入`paddle.fluid.embedding`的参数矩阵中？

+ 答复：需将本地词典向量读取为NumPy数据格式，然后使用`paddle.fluid.initializer.NumpyArrayInitializer`这个op初始化`paddle.fluid.embedding`里的`param_attr`参数，即可实现加载用户自定义（或预训练）的Embedding向量。

------

##### 问题：如何实现网络层中多个feature间共享该层的向量权重？

+ 答复：你可以使用`paddle.fluid.ParamAttr`并设定一个name参数，然后再将这个类的对象传入网络层的`param_attr`参数中，即将所有网络层中`param_attr`参数里的`name`设置为同一个，即可实现共享向量权重。如使用embedding层时，可以设置`param_attr=paddle.fluid.ParamAttr(name="word_embedding")`，然后把`param_attr`传入embedding层中。
...
+ 答复：使用`paddle.fluid.io.save_vars`保存指定的vars，然后使用`paddle.fluid.io.load_vars`加载对应层的参数值。具体示例请见API文档：[load_vars](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/api_cn/io_cn/load_vars_cn.html) 和 [save_vars](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/api_cn/io_cn/save_vars_cn.html) 。
...
##### 问题：`paddle.fluid.layers.pool2d`的全局池化参数和设置参数有关系么？
...
+ 答复：可以通过`param_attr`设置参数的属性，`paddle.fluid.ParamAttr(initializer=paddle.fluid.initializer.Normal(0.0, 0.02), learning_rate=2.0)`，如果`learning_rate`设置为0，该层就不参与训练。也可以构造一个numpy数据，使用`paddle.fluid.initializer.NumpyArrayInitializer`来给权重设置想要的值。
