...
##### 问题：请问`paddle.fluid.layers.matmul`和`paddle.fluid.layers.mul`有什么区别？
...
+ 答复：在`ParamAttr`里设置learning_rate=0或trainable设置为False。具体请参考文档：https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/api_cn/fluid_cn/ParamAttr_cn.html
...
##### 问题：多卡训练时启动失败，`Error：Out of all 4 Trainers`，如何处理？  @巩伟
...
详细请参考官方文档[存储分配与优化](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/advanced_guide/performance_improving/singlenode_training_improving/memory_optimize.html) 调整相关配置。
...
+ 答复：有如下两点建议：
  ...
  1. 如果数据预处理耗时较长，可使用DataLoader加速数据读取过程，具体请参考API文档：[paddle.fluid.io.DataLoader](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/api_cn/io_cn/DataLoader_cn.html#dataloader)。
...
+ 答复：请先参考[显存分配与优化文档](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/advanced_guide/performance_improving/singlenode_training_improving/memory_optimize.html) 开启存储优化开关，包括显存垃圾及时回收和Op内部的输出复用输入等。若存储空间仍然不够，建议：
...
+ 答复：CUDAPlace()接口需要指定GPU的ID编号，接口使用方法参见：[paddle.fluid.CUDAPlace](https://www.paddlepaddle.org.cn/documentation/docs/zh/1.8/api_cn/fluid_cn/CUDAPlace_cn.html#cudaplace)。
