...
We also provide methods that can be **compiled from local source code**, but since the situation on this machine is more complicated, we only provide support for specific systems.
...
8. Execute cmake:
	...
	> For the meaning of the specific [compilation options table](../Tables.html/#Compile), please refer to the compilation option table. Please note that the parameter `-DPY_VERSION` is the python version used in your current environment.
...
9. Execute cmake:
	...
	* For users who need to compile **GPU version PaddlePaddle**: (*only support ubuntu16.04/14.04*)
