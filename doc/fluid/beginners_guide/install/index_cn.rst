...
7. 如果您希望使用 `pip <https://pypi.org/project/pip/>`_ 进行安装PaddlePaddle可以直接使用以下命令:
    ...
    (2). **GPU版本** ：如果您想使用GPU版本请参考如下命令安装（使用清华源） 
        ...
        * 如果您是使用 Python 2，CUDA 8，cuDNN 7.1+，安装GPU版本的命令为：
        ::
            python -m pip install paddlepaddle-gpu==1.6.post87 -i https://pypi.tuna.tsinghua.edu.cn/simple

        * 如果您是使用 Python 2，CUDA 9，cuDNN 7.3+，安装GPU版本的命令为：
        ::
            python -m pip install paddlepaddle-gpu==1.6.post97 -i https://pypi.tuna.tsinghua.edu.cn/simple

        * 如果您是使用 Python 2，CUDA 10.0，cuDNN 7.3+，安装GPU版本的命令为：
        ::
            python -m pip install paddlepaddle-gpu==1.6.post107 -i https://pypi.tuna.tsinghua.edu.cn/simple
...
1. **CPU 版本**
    ...
    ::
        ...
        docker pull hub.baidubce.com/paddlepaddle/paddle:1.6

    (3). 用镜像构建并进入Docker容器：
    ::

        docker run --name paddle -it -v dir1:dir2 hub.baidubce.com/paddlepaddle/paddle:1.6 /bin/bash
        ...
        > hub.baidubce.com/paddlepaddle/paddle:1.6 是需要使用的image名称；/bin/bash是在Docker中要执行的命令
...
2. **GPU 版本**
    ...
    ::
        ...
        nvidia-docker pull hub.baidubce.com/paddlepaddle/paddle:1.6-gpu-cuda10.0-cudnn7

    (3). 用镜像构建并进入Docker容器：
    ::

        nvidia-docker run --name paddle -it -v dir1:dir2 hub.baidubce.com/paddlepaddle/paddle:1.6-gpu-cuda10.0-cudnn7 /bin/bash
        ...
        > hub.baidubce.com/paddlepaddle/paddle:1.6-gpu-cuda10.0-cudnn7 是需要使用的image名称；/bin/bash是在Docker中要执行的命令  
...
3. 如果您的机器不在中国大陆地区，可以直接从DockerHub拉取镜像：
    ::
        ...
        docker run --name paddle -it -v dir1:dir2 paddlepaddle/paddle:1.6 /bin/bash
        ...
        > paddlepaddle/paddle:1.6 是需要使用的image名称；/bin/bash是在Docker中要执行的命令
