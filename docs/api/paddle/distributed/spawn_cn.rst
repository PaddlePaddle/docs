.. _cn_api_paddle_distributed_spawn:
...
:::::::::
    ...
    - **args** (list|tuple，可选) - 传入目标函数 ``func`` 的参数。
    - **nprocs** (int，可选) - 启动进程的数目。默认值为-1。当 ``nprocs`` 为 -1 时，模型执行时将会从环境变量中获取当前可用的所有设备进行使用：如果使用 GPU 执行任务，将会从环境变量 ``CUDA_VISIBLE_DEVICES`` 中获取当前所有可用的设备 ID；如果使用 XPU 执行任务，将会从环境变量 ``XPU_VISIBLE_DEVICES`` 中获取当前所有可用的设备 ID。
