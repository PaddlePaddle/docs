...
基础参数
:::::::::
    - ``--master``: 主节，支持缺省 http:// 和 etcd://，认缺省 http://。例如 ``--master=127.0.0.1:8080``. 默认值 ``--master=None``.

    - ``--rank``: 节点序，可以通过主节点进行分配。默认值 ``--rank=-1``.

    - ``--log_level``: 日志级，可选值为 CRITICAL/ERROR/WARNING/INFO/DEBUG/NOTSET，区分大小写。默认值 ``--log_level=INFO``.
...
:::::::::
    ...
    - ``--heter_worker_num``: 在异构集群中启动单机模拟分布式任，指定参数服务器异构训练节点的个数。
...
:::::::::
    ...
    - ``--elastic_level``: 弹性级别设置，-1: 不开，0: 错误节点退，1: 节点内重启. 默认值 ``--elastic_level=-1``.
...
.. code-block:: bash
    ...
    # 在多机上启，例如在 192.168.0.16, 192.168.0.17 分别启动1个 server 和2个 trainer
