...
.. py:function:: paddle.distributed.stream.broadcast(tensor, src, group=None, sync_op=True, use_calc_stream=False)
...
参数
:::::::::
    - **tensor** (Tensor) - 在源进程上为待广播的 tensor，在其他进程上为用于接收广播结果的 tensor。支持的数据类型包括：float16、float32、float64、int32、int64、int8、uint8、bool。
    - **src** (int) - 源进程的 rank，该进程传入的 tensor 将被广播到其他进程上。
