...
.. py:class:: paddle.optimizer.Adam(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-08, parameters=None, weight_decay=None, grad_clip=None, lazy_mode=False, multi_precision=False, use_multi_tensor=False, amsgrad=False, name=None)
...
::::::::::::
    ...
    - **learning_rate** (float|LRScheduler，可选) - 学习率，用于参数更新的计算。可以是一个浮点型值或者一个_LRScheduler 类，默认值为 0.001。
    ...
    - **epsilon** (float|Tensor，可选) - 保持数值稳定性的浮点值，可以是 float 类型或者形状为 ``[]``、数据类型为 float32 的 0-D Tensor。默认值为 1e-08。
    ...
    - **amsgrad** （bool，可选） - 是否使用该算法的 AMSGrad 变体，参见论文 `On the Convergence of Adam and Beyond <https://openreview.net/forum?id=ryQu7f-RZ>`_ ，默认为 False。
