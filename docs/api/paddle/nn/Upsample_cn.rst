...
:::::::::
    ...
    - **size** (list|tuple|Tensor|None，可选) - 输出 Tensor 的形状，输入为 3D 张量时，形状为为(out_w)的 1-D Tensor。输入为 4D 张量时，形状为为(out_h, out_w)的 2-D Tensor。输入为 5-D Tensor 时，形状为(out_d, out_h, out_w)的 3-D Tensor。如果 :code:`size` 是列表，每一个元素可以是整数或者形状为[1]的变量。如果 :code:`size` 是变量，则其维度大小为 1。默认值为 None。
