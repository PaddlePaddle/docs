...
::::::::::::
    ...
    - **param_attr** (ParamAttr|bool|None，可选) - 指定权重参数属性的对象。默认值为 None，表示使用默认的权重参数属性。具体用法请参见 :ref:`cn_api_paddle_ParamAttr` 。
    - **bias_attr** (ParamAttr|bool|None，可选) - 指定偏置参数属性的对象。默认值为 None，表示使用默认的偏置参数属性。具体用法请参见 :ref:`cn_api_paddle_ParamAttr` 。
    ...
    - **do_model_average_for_mean_and_var** (bool，可选) - 指示是否为 mean 和 variance 做模型均值。默认值：True。
