...
形状
::::::::
    - **输入**  ： N-D Tensor，维度是[N，*] 其中 N 是 batch size，`*` 是任意数量的额外维度，数据类型为 float32 或 float64。
    - **其他**  ： 与 输入 的形状、数据类型相同。
    - **label**  ： 与 输入 的形状、数据类型相同。
    - **输出**  ： 如果 :attr:`reduction` 为 ``'sum'`` 或者是 ``'mean'``，则形状为 :math:`[1]`，否则 shape 和输入 `input` 保持一致。数据类型与 ``input``、 ``other`` 相同。
