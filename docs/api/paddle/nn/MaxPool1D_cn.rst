...
参数
:::::::::
    - **kernel_size** (int|list|tuple): 池化核的尺寸大小. 如果kernel_size为list或tuple类，其必须包含一个整，最终池化核的大小为该数值。
    - **stride** (int|list|tuple，可选): 池化操作步长. 如果stride为list或tuple类，其必须包含一个整数，最终池化操作的步长为该数值。默认值为None，时会使用kernel_size作为stride。
    ...
    - **ceil_mode** (bool，可选): 是否用ceil函数计算输出的height和width，如果设置为False，使用floor函数来计算，默认为False。
