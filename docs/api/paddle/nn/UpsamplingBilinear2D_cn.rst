...
::::::::::::
    ...
    - **size** (int|list|tuple|Tensor|None) - 输出 Tensor，输入为 4D Tensor，形状为为(out_h, out_w)的 2-D Tensor。每一个元素可以是整数或者形状为[]的 0-D Tensor。如果 :code:`size` 是一个整形数，则 `out_h` 和 `out_w` 都为这个数。如果 :code:`size` 是列表，每一个元素可以是整数或者形状为[]的 0-D Tensor。如果 :code:`size` 是 Tensor，则其为 1D Tensor。默认值为 None。
