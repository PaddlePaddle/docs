...
:::::::::
    ...
    - **label** (Tensor) – 输入input对应的标签值。若soft_label=False，要求label维度为 :math:`[N_1, N_2, ..., N_k]` 或 :math:`[N_1, N_2, ..., N_k, 1]` ，数据类型为'int64'，且值必须大于等于0且小于C；若soft_label=True，数据类型可以为'float32', 'float64'，要求label的维度、数据类型与input相同，且每个样本各软标签的总和为1。
