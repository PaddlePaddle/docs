...
该OP默认会对结果进行求mean计，您也可以影响该默认行为，具体参考reduction参数说明。
...
如果 ``weight`` 参数为 ``None`` ，直接进入下一步。

如果 ``weight`` 参数不为 ``None`` ，对每个样本的交叉熵进行weight加权(区分soft_label = False or True):
...
2.3 如果 ``reduction`` 参数为 ``mean``，根据 ``weight``  参数情况进行处理:  
...
2.3.2. 如果 ``weight`` 参数不为 ``None`` ，返回上一步结果的加权平均值
