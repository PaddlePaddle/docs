...
参数
:::::::::
  - **x**  (Tensor) – 时移算符的输入Tensor。维度为 :math:`[N*T，C，H，W]` 的 4-D Tensor。N 为批量大小，T 为时间段数，C 为信道数，H 为特征高度，W 为特征宽度，数据类型为 float32 或 float64。
...
返回
:::::::::
Tensor，时序位移后的输出Tensor，维度和数据类型与输入 ``x`` 一致。
