...
..  note::
    详细请参考对应的 ``Class`` ：:ref:`cn_api_paddle_nn_AdaptiveMaxPool2D` 。


参数
:::::::::
    - **x** (Tensor)：当前算子的输入，其是一个形状为 ``[N, C, H, W]`` 的 4-D Tensor。其中 ``N`` 是 batch size, ``C`` 是通道数，``H`` 是输入特征的高度，``W`` 是输入特征的宽度。其数据类型为 float32 或者 float64。别名 ``input``。
