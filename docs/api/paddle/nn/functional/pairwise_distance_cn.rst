...
参数
::::::::
    - **x** (Tensor) - 形状为 :math:`[N, D]` 或 :math:`[D]` 的Tensor，其中 :math:`N` 是批大小，:math:`D` 是向量的维度，数据类型为 float32 或 float64。
    - **y** (Tensor) - 形状为 :math:`[N, D]` 或 :math:`[D]` 的Tensor，其中 :math:`N` 是批大小，:math:`D` 是向量的维度，数据类型为 float32 或 float64。
