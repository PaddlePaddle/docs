...
参数
:::::::::
    - **x** (Tensor)：当前算子的输入，其是一个形状为 ``[N, C, D, H, W]`` 的 5-D Tensor。其中 ``N`` 是 batch size, ``C`` 是通道数， ``D`` 是输入特征的深度， ``H`` 是输入特征的高度， ``W`` 是输入特征的宽度。其数据类型为 ``float16``, ``bfloat16``, ``float32``, ``float64`` 。别名 ``input``。
