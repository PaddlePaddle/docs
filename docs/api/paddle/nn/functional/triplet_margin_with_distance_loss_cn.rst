...
参数
:::::::::
    - **input** (Tensor) - :math:`[N, * ]` ，中N是batch_size，`*` 是任意其他维度。数据类型是float32、float64。
    - **positive** (Tensor) - :math:`[N, *]` ，正样本，维度、数据类型与输入 ``input`` 相同。
    - **negative** (Tensor) - :math:`[N, *]` ，负样本，维度、数据类型与输入 ``input`` 相同。
    - **distance_function** (Callable，可选) - 手动指定范数，默认为 None，算欧式距离。
...
返回
:::::::::
    输出的Tensor。如果 :attr:`reduction` 是 ``'none'``，输出的维度为 :math:`[N, *]` ，与输入 ``input`` 的形状相同。如果 :attr:`reduction` 是 ``'mean'`` 或           ``'sum'``，则输出的维度为 :math:`[1]` 。
