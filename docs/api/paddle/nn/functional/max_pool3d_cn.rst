...
参数
:::::::::
    - **x** (Tensor) - 形状为 [N,C,D,H,W] 或 [N,D,H,W,C] 的 5-D Tensor，N 是批尺寸，C 是通道数，D 是特征深度，H 是特征高度，W 是特征宽度，数据类型为 float32 或 float64。别名 ``input``。
    ...
    - **return_mask** (bool) - 是否返回最大索引和输出。默认为 False。别名 ``return_indices``。
    ...
    - **data_format** (str) - 输入和输出的数据格式，可以是"NCDHW"和"NDHWC"。N 是批尺寸，C 是通道数，D 是特征深度，H 是特征高度，W 是特征宽度。默认值："NCDHW"。
