...
.. py:function:: paddle.nn.functional.dropout1d(input, p=0.5, training=True, inplace=False)
...
参数
:::::::::
 - **input** (Tensor) - 形状为 ``[N, C, L]`` 的 3D Tensor 或 ``[C, L]`` 的 2D Tensor，数据类型为 float16、float32 或 float64。
...
返回
:::::::::
经过 dropout1d 之后的结果，与输入 ``input`` 形状相同的 ``Tensor``。
