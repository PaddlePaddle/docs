...
该OP基于flow field网格的对输入X进行双线性插值采样。网格通常由affine_grid生，shape为[N, H, W, 2]，是shape为[N, H, W]的采样点张量的(x, y)坐标。
...
::::::::::::
  ...
  - **x** (Tensor): 输入张量，维度为 :math:`[N, C, H, W]` 的4-D Tensor，N为批尺寸，C是通道数，H是特征高度，W是特征宽，数据类型为float32或float64。
  - **grid** (Tensor): 输入网格数据张量，维度为 :math:`[N, H, W, 2]` 的4-D Tensor，N为批尺寸，H是特征高度，W是特征宽，数据类型为float32或float64。
...
返回
::::::::::::
Tensor，入X基于输入网格的双线性插值计算结果，维度为 :math:`[N, C, H, W]` 的4-D Tensor，据类型与 ``x`` 一致。
