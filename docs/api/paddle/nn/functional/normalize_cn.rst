.. _cn_api_paddle_nn_functional_normalize:

normalize
-------------------------------

.. py:function:: paddle.nn.functional.normalize(x, p=2, axis=1, epsilon=1e-12, out=None, name=None)
...
参数
:::::::::
    - **x** (Tensor) - 输入可以是 N-D Tensor。数据类型为：float32、float64。别名 ``input``。
    - **p** (float|int，可选) - 范数公式中的指数值。默认值：2
    - **axis** (int，可选) - 要进行归一化的轴。如果 ``x`` 是 1-D Tensor，轴固定为 0。如果 ``axis < 0``，轴为 ``x.ndim + axis``。-1 表示最后一维。别名 ``dim``。
    - **epsilon** (float，可选) - 添加到分母上的值以防止分母为 0。默认值为 1e-12。别名 ``eps``。
