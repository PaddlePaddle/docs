.. _cn_api_paddle_nn_functional_lp_pool2d:
...
.. note::
    ...
    1. ``paddle.nn.functional.lp_pool2d(x, norm_type, kernel_size, stride=None, padding=0, ceil_mode=False, data_format='NCHW', name=None)`` （Paddle 风格）。
    2. ``paddle.nn.functional.lp_pool2d(input, norm_type, kernel_size, stride=None, ceil_mode=False)`` （PyTorch 风格）。
...
参数
:::::::::
    - **x** (Tensor)：形状为 ``[N,C,H,W]`` 或 ``[N,H,W,C]`` 的 4-D Tensor，N 是批尺寸，C 是通道数，H 是特征高度，W 是特征宽度，数据类型为 float32 或 float64。别名 ``input``。
