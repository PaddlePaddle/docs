...
该 api 计算输入 ``input``、``positive`` 和 ``negative`` 间的 ``triplet margin loss`` 损失，测量 ``input`` 与 ``positive examples`` 和 ``negative examples`` 之间的相对相似性。所有输入 Tensor 的形状都为 :math:`(N, *)`，``*`` 是任意其他维度。
