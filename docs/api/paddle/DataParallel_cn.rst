...
.. py:class:: paddle.DataParallel(layers, strategy=None, comm_buffer_size=25, last_comm_buffer_size=1, find_unused_parameters=False, group=None)
...
::::::::::::
    ...
    - **last_comm_buffer_size** （float，可选）- 它限制通信调用中最后一个缓冲区的内存大小（MB）。减小最后一个通信缓冲区的大小有助于提高性能。默认值：1。
    ...
    - **group** (Group，可选) - 数据并行通信使用的进程组。默认值为 None。
...
**参数**
    ...
    - **structured_name_prefix** (str，可选) - 添加到子层结构化名称前的前缀。默认值为 ``""``。
