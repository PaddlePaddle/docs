...
根据参数 `offset`、`axis1`、`axis2`，回输入 `Tensor` 的局部视图。
...
参数
:::::::::
    - **x** (Tensor) : 输入变量，类型为Tensor，持bool、int32、int64、float16、float32、float64数据类型。
...
返回
:::::::::
    - Tensor (Tensor)，入 `Tensor` 在指定二维平面的局部视图，数据类型和输入数据类型一致。
