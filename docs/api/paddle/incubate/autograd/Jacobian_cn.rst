...
的输出为一个二维(不包含batch)或三维(包含batch，零维为batch)的Tensor。

例如，假设 ``is_batched=True``，输入Tensor经过展平并拼接后的形状为 ``(B, M)`` ，出
Tensor经过展平并拼接后的形状为 ``(B, N)`` ，最终输出雅可比矩阵形状为 ``(B, M, N)`` . 
...
例如，假设 ``Jacobian`` 的实例 ``J`` 形状为 ``(B, M, N)`` ，设 ``M>4`` , 
则 ``J[:, 1:4:1, :]`` 表示获取 ``J`` 的第 ``1`` 行到第 ``3`` 行值 ，际计算时，仅会对
