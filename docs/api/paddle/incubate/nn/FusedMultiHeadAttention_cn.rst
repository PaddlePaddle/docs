...
（2）q, k, v 的 weight 的存储格式不同。``MultiHeadAttention`` 将 q, k, v 的 weight 存储在三个Tensor中。``FusedMultiHeadAttention`` 的 q, k, v 的 weight 被统一存在一个权重Tensor中，其维度为 ``[3, num_heads, head_dim, embed_dim]`` 。
