...
::::::::::::
    ...
    - **loc** (scalar，可选) - 分布峰值的位置参数，数据类型为 float32 或 float64。默认值为 0，此时分布对称中心位于 `x` = 0 处。
    - **scale** (scalar，可选) - 表示分布峰值一半高度处的半宽度（HWHM），控制曲线的陡峭程度。必须为正数，数据类型为 float32 或 float64。默认值为 1，表示柯西分布的概率密度函数在 `x` = ±1 处的高度为 1。
