...
::::::::::::
        ...
        - **expand_times** （list|tuple|Variable）- 数据类型是 ``int32``。如果 ``expand_times`` 的类型是 list 或 tuple，它的元素可以是整数或者形状为[]的 ``0-D Tensor``。如果 ``expand_times`` 的类型是 ``Variable``，则是 1-D ``Tensor``。表示 ``x`` 每一个维度被复制的次数。
