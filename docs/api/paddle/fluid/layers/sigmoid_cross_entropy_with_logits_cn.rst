...
::::::::::::
  ...
  - **x** (Variable) - (Tensor，默认 Tensor<float>)，形为 N x D 的二维Tensor，N为batch大小，D为类别数目。该输入是一个由先前运算得出的logit组成的Tensor。logit是未标准化(unscaled)的log概率，公式为 :math:`log(\frac{p}{1-p})`，数据类型为float32或float64。
  - **label** (Variable) -  (Tensor，默认 Tensor<float>) 具有和 ``X`` 相同数据类型，相同形状的二维Tensor。该输入Tensor代表了每个logit的可能标签。
...
返回
::::::::::::
 Variable(Tensor，默认Tensor<float>)，形为 N x D 的二维Tensor，其值代表了按元素的logistic loss，数据类型为float32或float64。
