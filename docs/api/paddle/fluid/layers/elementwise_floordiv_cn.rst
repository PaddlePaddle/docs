...
该 OP 是逐元素整除算子，输入 ``x`` 与输入 ``y`` 逐元素整除，并将各个位置的输出元素保存到返回结果中。
...
::::::::::::
        ...
        - **x** （Variable）- 多维 ``Tensor``。数据类型为 ``int32`` 或  ``int64``。
        - **y** （Variable）- 多维 ``Tensor``。数据类型为 ``int32`` 或  ``int64``。
        - **axis** （int32，可选）-  ``y`` 的维度对应到 ``x`` 维度上时的索引。默认值为 -1。
        - **act** （str，可选）- 激活函数名称，作用于输出上。详细请参考 :ref:`api_guide_activations`，常见的激活函数有：``relu`` ``tanh`` ``sigmoid`` 等。如果为 None 则不添加激活函数。默认值为 None。
