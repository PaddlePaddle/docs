...
::::::::::::
  ...
  - **input** （Variable） - 池化操作的输入Tensor，维度为 :math:`[N, C, D, H, W]` 的5-D Tensor。输入Tensor的格式为NCDHW，其中N是batch大小，C是通道数，D为特征的深度，H是特征的高度，W是特征的宽度，数据类型为float32或float64。
