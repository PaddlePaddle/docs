...
如果提供 ``score_threshold`` 阈值且 ``nms_top_k`` 大于-1，则选择置信度分数最大的k个框。 然后按照Matrix NMS算法对分数进行衰减。经过抑制后，如果 ``keep_top_k`` 大于-1，每张图片最终保留 ``keep_top_k`` 个检测框。
...
::::::::::::
    ...
    - **keep_top_k**  (int) – 经过NMS抑制，最终保留的最大检测次数。如果设置为 -1 ，则则保留全部。
