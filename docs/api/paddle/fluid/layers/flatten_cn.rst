...
::::::::::::
  ...
  - **axis** (int) - flatten展开的分割轴，[0, axis) 轴数据被flatten到输出矩阵的0轴，[axis, R)数据被flatten到输出矩阵的1轴，其中R是输入Tensor的总维度数。axis的值必须在[0,R]范围内。当 axis=0 时，若输入Tensor的维度为 :math:`[d_0, d_1，… d_n]`，则输出Tensor的Tensor维度为 :math:`[1，d_0 * d_1 *… d_n]`，默认值为1。
