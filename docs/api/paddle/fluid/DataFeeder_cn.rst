...
reader通常返回一个minibatch条目列表。在列表中每一条目都是一个样本（sample），是由具有一至多个特征的列表或元组组成的。
...
::::::::::::
    ...
    - **place** (Place) – place表明是向GPU还是CPU中输入数据。如果想向GPU中输入数，请使用 ``fluid.CUDAPlace(i)`` (i 代表 the GPU id)；如果向CPU中输入数，请使用  ``fluid.CPUPlace()``
    - **program** (Program) – 需要向其中输入数据的Program。如果为None，默认使用 ``default_main_program()``。 缺省值为None
...
**参数**
        ...
        - **num_places** (int) – 如果 ``multi_devices`` 为 ``True`` ，以使用此参数来设置GPU数目。如果 ``multi_devices`` 为 ``None`` ，该函数默认使用当前训练机所有GPU设备。默认为None。
