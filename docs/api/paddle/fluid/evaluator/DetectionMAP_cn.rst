...
::::::::::::
    ...
    - **gt_difficult** (Variable|None，选) – 指定这个ground truth是否是一个difficult bounding bbox，它可以是一个 shape=[N, 1]的LoDTensor，也可以不被指定。默认设置为None，表示所有的ground truth标签都不是difficult bbox，数据类型为float32或float64。
...
**参数**
    ...
    - **reset_program** (Program|None，选) – 单个program 的 reset 过程。如果设置为 None，将创建一个 program
