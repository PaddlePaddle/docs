...
::::::::::::
    ...
    - **weight** (Tensor) – 卷积核参数，形状为 :math:`[[M, C/g, kH, kW]`，中 M 是输出通道数，g 是group组，kH是卷积核高度尺寸，kW是卷积核宽度尺寸。数据类型为float32或float64。
    - **bias** (Tensor，选) - 可变形卷积偏置，形状为 :math:`[M,]` 。
    ...
    - **mask** (Tensor，可选) – 可变形卷积层的输入掩码，当使用可变形卷积算子v1时，请将mask设置为None，据类型为float32或float64。
