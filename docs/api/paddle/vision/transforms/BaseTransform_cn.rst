...
如果你想要定义自己的图像变化方，需要重写子类中的 ``_apply_*`` 方法。

参数
:::::::::

    - keys (list[str]|tuple[str], optional) - 输入的类型. 你的输入可以是单一的图像，也可以是包含不同数据结构的元，``keys`` 可以用来指定输入类型. 举个例，如果你的输入就是一个单一的图像，那么 ``keys`` 可以为 ``None`` 或者 ("image")。如果你的输入是两个图像：``(image, image)`` ，那么 `keys` 应该设置为 ``("image", "image")`` 。如果你的输入是 ``(image, boxes)``，么 ``keys`` 应该为 ``("image", "boxes")`` 。目前支持的数据类型如下所示:

            - "image": 输入的图，它的维度为 ``(H, W, C)`` 。 
            - "coords": 输入的左，它的维度为 ``(N, 2)`` 。 
            - "boxes": 输入的矩形，他的维度为 (N, 4)，式为 "xyxy"，一个 "xy" 表示矩形框左上方的坐，第二个 "xy" 表示矩形框右下方的坐标.
