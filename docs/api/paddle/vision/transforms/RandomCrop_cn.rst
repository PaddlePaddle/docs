...
:::::::::
    ...
    - **padding_mode** (string，可选) - 填充模式。支持：constant, edge, reflect 或 symmetric。默认值：constant。
        ...
        ``reflect`` 表示使用原图像的镜像值进行填充（不使用边缘上的值）；
        比如：使用该模式对 [1, 2, 3, 4] 的两端分别填充 2 个值，结果是 [3, 2, 1, 2, 3, 4, 3, 2]。
        ``symmetric`` 表示使用原图像的镜像值进行填充（使用边缘上的值）；
        比如：使用该模式对 [1, 2, 3, 4] 的两端分别填充 2 个值，结果是 [2, 1, 1, 2, 3, 4, 4, 3]。
