...
参数
:::::::::
    - **enable** (bool，选) - 是否使用loss scaling。默认值为True。
    - **init_loss_scaling** (float，选) - 初始loss scaling因子。默认值为32768.0。
    - **incr_ratio** (float，选) - 增大loss scaling时使用的乘数。默认值为2.0。
    - **decr_ratio** (float，选) - 减小loss scaling时使用的小于1的乘数。默认值为0.5。
    - **incr_every_n_steps** (int，选) - 连续n个steps的梯度都是有限值时，增加loss scaling。默认值为1000。
    - **decr_every_n_nan_or_inf** (int，选) - 累计出现n个steps的梯度为nan或者inf时，减小loss scaling。默认值为2。
    - **use_dynamic_loss_scaling** (bool，选) - 是否使用动态的loss scaling。如果不使用，则使用固定的loss scaling；如果使用，则会动态更新loss scaling。默认值为True。
