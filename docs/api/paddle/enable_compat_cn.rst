...
.. py:function:: paddle.enable_compat(*, scope=None, blocked_modules=None, backend='torch', silent=False, level=1)
...
:::::::::
    ...
    - **level** (int，可选) - 兼容级别。``1``（默认值）保留原有的 ``torch -> paddle`` 代理行为；``2`` 会将与 PyTorch 对齐的 ``paddle.compat.*`` API 别名到 ``paddle.*`` 和 ``paddle.Tensor``；``3`` 会同时启用上述两者。默认值为 ``1``。
