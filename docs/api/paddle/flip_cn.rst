...
::::::::::::
    ...
    - **axis** (list|tuple|int) - 需要翻转的轴。别名 ``dims``。当 axis 是 int 型时，且 axis<0，实际的计算维度为 ndim(x) + axis；axis 是 list 或 tuple 时，``axis[i] < 0`` 时，实际的计算维度为 ndim(x) + axis[i]，其中 i 为 axis 的索引。
