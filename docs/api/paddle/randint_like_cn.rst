...
.. py:function:: paddle.randint_like(x, low=0, high=None, dtype=None, name=None, *, device=None, pin_memory=False, requires_grad=False)
...
参数
::::::::::
    - **x** (Tensor) – 输入的多维 Tensor，数据类型可以是 bool，int32，int64，float16，float32，float64。输出 Tensor 的形状和 ``x`` 相同。如果 ``dtype`` 为 None，则输出 Tensor 的数据类型与 ``x`` 相同。别名 ``input``。
...
关键字参数
::::::::::
    - **device** (PlaceLike|None，可选) - 返回 Tensor 的期望设备。为 None 时，默认使用 ``x`` 所在设备。
