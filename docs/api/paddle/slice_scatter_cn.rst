...
.. py:function:: paddle.slice_scatter(x, value, axes, starts, ends, strides)
...
:::::::::
    ...
    - **axes**  (list|tuple) - 指定沿着哪几个维度嵌入对应的值。别名 ``dim``。
    - **starts**  (list|tuple) - 嵌入的起始索引。别名 ``start``。
    - **ends**  (list|tuple) - 嵌入的截止索引。别名 ``end``。
    - **strides**  (list|tuple) - 嵌入的步长。别名 ``step``。
