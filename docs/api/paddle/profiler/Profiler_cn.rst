...
:::::::::
    ...
    - **timer_only** (bool，选) - 如果设置为True，将只统计模型的数据读取和每一个迭代所消耗的时间，而不进行性能分析。否则，模型将被计时，同时进行性能分析。默认值：False。
...
性能分析器状态从CLOSED -> scheduler(0)，根据新的状态触发相应行为。
