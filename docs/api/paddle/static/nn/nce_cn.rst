...
::::::::::::
    ...
    - **input** (Tensor) -  输入张，2-D 张量，形状为 [batch_size, dim]，数据类型为 float32 或者 float64。
    ...
    - **is_sparse** (bool，可选) – 标志位，指明是否使用稀疏更，为 ``True`` 时 :math:`weight@GRAD` 和 :math:`bias@GRAD` 的类型会变为 SelectedRows。默认为 ``False`` 。
