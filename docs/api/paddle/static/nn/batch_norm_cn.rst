.. _cn_api_paddle_static_nn_batch_norm:

batch_norm
-------------------------------


.. py:function:: paddle.static.nn.batch_norm(input, act=None, is_test=False, momentum=0.9, epsilon=1e-05, param_attr=None, bias_attr=None, data_layout='NCHW', in_place=False, name=None, moving_mean_name=None, moving_variance_name=None, do_model_average_for_mean_and_var=True, use_global_stats=False)
...
::::::::::::
    ...
    - **act**  (string) - 激活函数类型，可以是 linear、relu、prelu 等。默认：None。
    - **is_test**  (bool)  - 指示它是否在测试阶段，非训练阶段使用训练过程中统计到的全局均值和全局方差。默认：False。
    - **momentum**  (float|Tensor) - 此值用于计算 moving_mean 和 moving_var，是一个 float 类型或者一个 shape 为 ``[]`` 的 0-D、数据类型为 float32 的 Tensor。更新公式为：:math:`moving\_mean = moving\_mean * momentum + new\_mean * (1. - momentum)` ， :math:`moving\_var = moving\_var * momentum + new\_var * (1. - momentum)`，默认：0.9。
    ...
    - **do_model_average_for_mean_and_var**  (bool，默认 True) - 是否为 mean 和 variance 做模型均值。
