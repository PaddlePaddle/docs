...
.. py:function:: paddle.static.nn.sparse_embedding(input, size, padding_idx=None, is_test=False, entry=None, table_class="MemorySparseTable", param_attr=None, dtype='float32', slot=None)
...
::::::::
    ...
    - **table_class** (str，可选) - 稀疏表的类型，其值可以为 CommonSparseTable、SSDSparseTable 和 MemorySparseTable。默认为 MemorySparseTable。
    ...
    - **slot** (int|None，可选) - 稀疏查找表使用的 slot 属性。为 None 时使用 0。默认值为 None。
