...
有些 OP，在训练和测试之间的行为是不同的，比如 :ref:`cn_api_paddle_static_nn_batch_norm`。它们有一个属性 ``is_test`` 来控制行为。当 ``for_test=True`` 时，此方法将把它们的 ``is_test`` 属性更改为 True。
