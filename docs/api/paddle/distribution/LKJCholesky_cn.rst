...
.. py:class:: paddle.distribution.LKJCholesky(dim=2, concentration=1.0, sample_method='onion')
...
相关矩阵的下三角 Cholesky 因子的 LKJ 分布支持两种 sample 方法：``onion`` 和 ``cvine``。
...
log_prob(value)
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
计算给定相关矩阵 Cholesky 因子在 LKJ 分布下的对数概率密度。
...
**返回**
    ...
    - **Tensor** - 给定相关矩阵 Cholesky 因子对应的对数概率密度。
...
**参数**
    ...
    - **sample_shape** (Sequence[int]，可选) - 采样的样本维度。

**返回**

    - **Tensor** - 指定维度的样本数据。数据类型与 ``concentration`` 相同。
