...
:math:`\boldsymbol X \in R_k`，支撑集 :math:`\boldsymbol X \in (0,1), ||\boldsymbol X|| = 1` ，概率密度函数（pdf）为:
...
:math:`\alpha_0=\sum_{i=1}^{k} \alpha_i` 是分布参数的，:math:`\Gamma(\alpha)` 为
Gamma函数。
...
- **concentration** (Tensor) - 浓度参数，即上述公式 :math:`\alpha` 参数。当
  ...
  ``event_shape=concentration.shape[-1:]`` ，余维为Batch维，
...
- **shape** (Sequence[int]，选) - 采样次数，最终生成样本形状为 ``shape+batch_shape+event_shape`` .
