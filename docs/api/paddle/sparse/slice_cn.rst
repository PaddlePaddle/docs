...
.. py:function:: paddle.sparse.slice(x, axes, starts, ends, name=None)
...
如果传递给 ``starts`` 或 ``ends`` 的值大于 n (维度中的元素数目)，则表示 n。
...
参数
:::::::::
    - **x** (Tensor) - 输入的多维 ``Tensor``，数据类型为 bool、float16、float32、float64、int16、int32 或 int64。
    - **axes** (list|tuple|Tensor) - 数据类型是 ``int32``。如果 ``axes`` 的类型是 list 或 tuple, 它的元素可以是整数或者形状为[]的 0-D ``Tensor``。如果 ``axes`` 的类型是 ``Tensor``，则是 1-D ``Tensor``。表示进行切片的轴。
