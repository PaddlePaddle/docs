...
:::::::::
    ...
    - **shape** (tuple|list|Tensor|*shape) - 要创建的 Tensor 的形状，``shape`` 的数据类型为 int32 或 int64。若 shape 是 list 或 tuple 类型，其元素应为整数或零维张量，若 shape 是 Tensor 类型，其元素应为表示列表的一维张量。 如果是 ``*shape``，则直接以可变长参数形式传入整数（例如 ``ones(2, 3)`` ）。别名： ``size``。
