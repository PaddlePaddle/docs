.. _cn_api_paddle_searchsorted:

searchsorted
-------------------------------

.. py:function:: paddle.searchsorted(sorted_sequence, values, out_int32=False, right=False, name=None, *, side=None, out=None, sorter=None)
...
::::::::
    ...
    - **values** (Tensor) - 输入的 N 维 Tensor，支持的数据类型：bfloat16、float16、float32、float64、int32、int64。别名 ``input``。
...
关键字参数
::::::::::
    - **side** (str|None，可选) - 与 ``right`` 作用相同，但推荐使用该参数。``"left"`` 对应 ``right=False``，``"right"`` 对应 ``right=True``；若其为 ``"left"`` 且 ``right=True``，会报错。默认值为 None。
    - **out** (Tensor|None，可选) - 输出 Tensor。默认值为 None。
    - **sorter** (Tensor|None，可选) - 可选的索引 Tensor，形状应与未排序的 ``sorted_sequence`` 相同；其索引序列应能在最后一维将 ``sorted_sequence`` 升序排序。
