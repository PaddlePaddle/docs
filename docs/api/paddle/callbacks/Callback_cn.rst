...
**参数**
    ...
    - **logs** (dict|None): 日志信息是dict或None. 通过paddle.Model传递的dict包含的字段有'loss'，估指标metric的名字，以及'batch_size'。
...
**参数**
    ...
    - **logs** (dict|None): 日志信息是dict或None. 通过paddle.Model传递的dict包含的字段有'steps'和'metrics'。'steps'是验证集的总共步长，'metrics'是一个list[str]，含'loss'和所设置的paddle.metric.Metric的名字。
...
**参数**
    ...
    - **step** (int): 训练步长或迭代次数。
    - **logs** (dict|None): 日志信息是dict或None. 通过paddle.Model传递的dict包含的字段有'loss'，估指标metric的名字，以及当前'batch_size'。
