...
::::::::::
    ...
    - **dtype** (str|np.dtype|paddle.dtype，可选) - 输出 Tensor 的数据类型，支持 float16、bfloat16、float32、float64。当该参数值为 None 时，输出 Tensor 的数据类型与输入 Tensor 的数据类型一致。默认值为 None。
    - **device** (str|paddle.Place，可选) - 指定创建 Tensor 的设备位置。如果为 None，则使用与输入 Tensor 相同的设备。默认值为 None。
    - **requires_grad** (bool，可选) - 是否为创建的 Tensor 计算梯度，默认值为 False，表示不计算梯度。
