...
::::::::::::
  ...
  - **grad_tensors** (None|list[Tensor|None]，选) – ``tensors`` 的初始梯度值。如果非None，须和 ``tensors`` 有相同的长度，并且如果其中某一Tensor元素为None，则该初始梯度值为填充1.0 的默认值；如果是None，所有的 ``tensors`` 的初始梯度值为填充1.0 的默认值。默认值：None。
