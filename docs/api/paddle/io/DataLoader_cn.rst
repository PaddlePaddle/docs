...
::::::::::::
    ...
    - **dataset** (Dataset | callable) - 待加载数据的数据集，支持两种格式:
      ...
      2. 由 :ref:`cn_api_io_map` 和 :ref:`cn_api_io_data_reader` 或 ``paddle.vision.ops`` 下的API组成的可执行函数，会使用C++端GPU多流、多线程数据加载器，当前仅支持ImageNet格式数据集加载，使用方法请见 :ref:`cn_api_io_map` 。
