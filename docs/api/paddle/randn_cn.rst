...
参数
::::::::::
  - **shape** (list|tuple|Tensor|*shape) - 生成的随机 Tensor 的形状。如果 ``shape`` 是 list、tuple，则其中的元素可以是 int，或者是形状为[]且数据类型为 int32、int64 的 0-D Tensor。如果 ``shape`` 是 Tensor，则是数据类型为 int32、int64 的 1-D Tensor。 如果是 ``shape`` 是 \*shape，则直接以可变长参数形式传入整数（例如 ``randn(2, 3)``）。别名： ``size``。
