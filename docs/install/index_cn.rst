...
* 新增对 CUDA 11.2 的支持，并不再支持 CUDA 9.0、CUDA 10.0 和 CUDA 11.0（如果您使用的是安培架构的GPU，推荐使用CUDA11.2；如果您使用的是非安培架构的GPU，推荐使用CUDA10.2，性能更优）
* CUDA 10.1 相关包新增对 gcc5.4 的支持
* 新增对 ROCm 平台的支持（2.1 中飞桨对 ROCm 平台的支持是 experimental 的）
* Linux系统相关的包已被拆分为 avx 和 noavx 两种类型的包（大部分机器都使用 avx 指令集，可在 `Linux下的PIP安装 <pip/linux-pip.html>`_ 页面通过命令查看您的机器是否支持）
