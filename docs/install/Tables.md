...
需要注意的是，命令中<code> paddlepaddle-gpu==2.6.1 </code> 在 windows 环境下，会默认安装支持 CUDA 11.8 和 cuDNN 8 的对应[版本号]的 PaddlePaddle 安装包
