...
控制流 `if/for/while` 的转写和处理是动转静中比较重要的模块，也是动态图模型和静态图模型实现上差别最大的一部分。如下图所示，对于控制流的转写分为两个阶段：转写期和执行期。在转写期，动转静模块将控制流语句转写为统一的形式；在执行期，根据控制流是否依赖 `Tensor` 来决定是否将控制流转写为相应的 `if_op/while_op` 。
...
+ **并非**所有动态图中的 `if/for/while` 都会转写为 `if_op/while_op`
...
无论是否会转写为 `if_op` ，动转静都会首先对代码进行处理，**转写为 `cond` 接口可以接受的写法**。
...
`convert_ifelse` 是框架底层的函数，在逐行执行用户代码生成 `Program` 时，执行到此处时，会根据**判断条件**的类型（ `bool` 还是 `Bool Tensor` ），自适应决定是否转为 `if_op` 。

```python
def convert_ifelse(pred, true_fn, false_fn, true_args, false_args, return_vars):

    if isinstance(pred, Value):  # 触发 cond_op 的转换
...
静态图模式下，神经网络会被描述为 `Program` 的数据结构，并对 `Program` 进行编译优化，再调用执行器获得计算结果。另外静态图的变量是 `Value` 类型（动态图是 `Tensor` 类型），因此要运行静态图模型，需要生成静态图的 `Program` 和 `Parameters`。

### 4.1 动态图 `layer` 生成 `Program`

文档开始的样例中 `forward` 函数包含两行组网代码：`Linear` 和 `add` 操作。以 `Linear` 为例，在飞桨框架底层，每个 API 在动态图和静态图下的行为是不同的：
...
class Linear(...):
    ...
    def forward(self, input):
        return _C_ops.matmul(input, self.weight, pre_bias, ...) # 这里会调用到下面的 ops_api.cc
```
...
static PyObject *matmul(PyObject *self, PyObject *args, PyObject *kwargs) {
  if (egr::Controller::Instance().GetCurrentTracer() == nullptr) {
    ...
    return static_api_matmul(self, args, kwargs);  // 静态图分支，会调用 PIR ApiBuilder 构建 pd_op.matmul OP
  } else {
    VLOG(6) << "Call eager_api_matmul";
    return eager_api_matmul(self, args, kwargs);   // 动态图分支，即直接执行 matmul kernel
  }
}
```

动态图 `layer` 生成 `Program`，其实是开启 `paddle.enable_static()` 时，在静态图下逐行执行用户定义的组网代码，依次添加 OP 到默认的主 Program（即 `main_program`）中。

### 4.2 动态图 `Tensor` 转为静态图 `Value`

上面提到，所有的组网代码都会在静态图模式下执行，以生成完整的 `Program`。**但静态图组网有一个前置条件必须满足**：

> **前置条件**：静态图组网时，所有的输入和输出必须都是静态图的 `Value` 类型，不能是动态图的 `Tensor` 类型。


**原因**：静态图下，操作的都是**IR 表示单元**：这包括了 `Program` 中计算相关的 `Operation` 以及数据相关的 `Value`。

因此，在动转静时，我们在需要在**某个统一的入口处**，将动态图 `Layers` 中 `Tensor` 类型（包含具体数据）的 `Weight`、`Bias` 等变量转换为**同名的静态图 `Value`**。

+ EagerParamBase &rarr; Parameters
+ Tensor &rarr; Value
...
+ `Paddle.nn.Layer` 基类的 `__call__` 函数
    ...
    def __call__(self, *inputs, **kwargs):
        # param_guard 会对将 Tensor 类型的 Param 和 buffer 转为静态图 Value
...
+ `_C_ops` 静态图参数解析阶段
    ...
    pir::Value CastPyArg2Value(PyObject* obj,
        ...
        obj = CastPyArg2ValuePreHook(obj);   // 通过 hook 将动态图 Tensor 转换为静态图 Value
...
+ 动态图 `layer` 调用在动转静时会走底层 `ApiBuilder` 的分支，以生成 `Program`
+ 动态图 `Tensor` 转为静态图 `Value`，并确保编译期的 `InferMeta` 正确执行
