...
def check_url_availability(url_info, timeout=10):
    ...
    try:
        ...
        return url_info, False, f"请求异常: {e!s}"
    except Exception as e:
        return url_info, False, f"未知错误: {e!s}"
    ...
    if unavailable_urls:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for url_info in unavailable_urls:
                ...
                f.write(
                    f"位置: 类别 {url_info['category_id']}.{url_info['category_name']} "
                )
                f.write(
                    f"第 {url_info['row_index']} 行 {url_info['column']}列\n"
                )
                f.write(f"上下文: {url_info['context']}\n")
                f.write(f"错误: {url_info['error']}\n")
                f.write("-" * 80 + "\n")
        print(
            f"生成 {output_path}，包含 {len(unavailable_urls)} 个不可用URL警告"
        )
    else:
