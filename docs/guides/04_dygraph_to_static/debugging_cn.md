...
```
执行后，报错日志如下图：
![图片](http://bos.bj.bce-internal.sdns.baidu.com/agroup-bos-bj/bj-4334746a0b9220b18f5f600fd0ac02957bb2754f)
报错日志从上到下一共可以分为4个部分：

- **原生的Python报错栈**
如1中的前两行所示，表示`/workspace/Paddle/run_dy2stat_error.py`文件第120行调用的函数`func2()`导致的后续一系列报错。

- **动转静报错栈起始标志位**
`In transformed code`，表示动转静报错信息栈，指运行转换后的代码时的报错信息。实际场景中，可以直接搜索`In transformed code`关键字，从这一行以下开始看报错日志即可。

- **用户代码报错栈**
隐藏了框架层面的无用的报错信息，突用户代码报错栈。并在出错代码下添加了波浪线和HERE指示词来提示具体的出错位置。并扩展了出错行代码上下文，帮助用户快速定位出错位置。
如上图3中所示，可以看出最后出错的用户代码为`x = paddle.reshape(x, shape=[1, two])`。

- **框架层面报错信息**
提供了静态图组网报错信息。一般可以直接根据最后三行的信息，定位具体是在生成哪个 OpDesc 时报的错误，一般是与执行此 Op 的 infershape 逻辑报的错误。
...
![图片](http://bos.bj.bce-internal.sdns.baidu.com/agroup-bos-bj/bj-d350a6821b9c9bfc2256befc56294c192ce3de85)
...
```
可以得到如下的报错信息：
![图片](http://bos.bj.bce-internal.sdns.baidu.com/agroup-bos-bj/bj-aebf5b24503f847f10f07cdfcc55e5a24d5f3119)
...
![图片](http://bos.bj.bce-internal.sdns.baidu.com/agroup-bos-bj/bj-1053f3c8a74911e30852360cc7535121c88123c4)
...
|命令|说明|
|:-:|:-:|
| l  | 查看当前位置的源代码; `l 3,20` 查看3~20行代码  |
| p expression| 在当前上下文中打印expression的值，如 `p x` 查看变量x|
| s | 执行下一行，进入函数内部 |
| n | 执行下一行，不会进入函数内部 |
| r | 执行代码到函数返回处 |
| b lineno | 在第lineno行处设置断点， 如`b 30`在30行处设置断点 |
| c | 执行代码，直到遇到下一个断点 |
| q | 退出pdb调试 |
...
![图片](http://bos.bj.bce-internal.sdns.baidu.com/agroup-bos-bj/bj-8e23a604885ebb1130076947c7ce875c8caa7ca5)
