...
从零开始构建变量，**Operator**，从而组建层，模型是一个很复杂的过程，并且当中难以避免的会出现很多冗余代码，因此**Paddle**提供了基础数据类型 ``paddle.nn.Layer`` ，来方便用户快速的实现自己的层和模型。模型和层都是基于 ``paddle.nn.Layer`` 的扩充实现，也因此可以说模型只是一种特殊的层。下面我们将演示如何利用 ``paddle.nn.Layer`` 建立自己的模型：
...
上述例子中我们创建并向模型中添加了一个名字为"img"的参数。随后可以直接通过调用**model.img**来访问该参数。
