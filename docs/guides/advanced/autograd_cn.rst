...
PaddlePaddle 的神经网络核心是自动微分，本篇文章主要介绍如何使用飞桨的自动微分，以及飞桨的自动微分机制，以便更好的使用飞桨进行训练。
...
为了让神经网络的判断更加准确，首先需要有衡量效果的工具，于是损失函数应运而生。如果想要神经网络的效果好，那么就要让损失函数尽可能的小，于是深度学习引入了能够有效计算函数最小值的算法–梯度下降等优化算法，以及参数优化更新过程–反向传播。
...
前向传播结束后，即可得到模型的预测结果\ ``predicts``\ ，这时可以使用飞桨中的对应损失函数 API 进行损失函数的计算。该例子中使用\ ``cross_entropy``\ 来计算损失函数，来衡量模型的预测情况。
...
随后进行反向传播，在飞桨中只需调用\ ``backward()``\ 即可自动化展开反向传播过程。各梯度保存在\ ``grad``\ 属性中。
...
本章主要介绍飞桨在实现反向传播进行自动微分计算时，内部是如何运行工作的。此部分为选读部分，更多是介绍飞桨内部实现机制，可以选择跳过，跳过不会影响正常使用。
...
在上面代码中\ ``c.backward()``\ 执行前，可以理解整个计算图是这样的：
