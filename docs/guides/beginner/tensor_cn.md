...
飞桨使用张量（[Tensor](../../../docs/api/paddle/tensor_cn.rst)） 来表示神经网络中传递的数据，Tensor 可以理解为多维数组，类似于 [Numpy 数组（ndarray）](https://numpy.org/doc/stable/user/quickstart.html#the-basics) 的概念。与 Numpy 数组相比，Tensor 除了支持运行在 CPU 上，还支持运行在 GPU 及各种 AI 芯片上，以实现计算加速；此外，飞桨基于 Tensor，实现了深度学习所必须的反向传播功能和多种多样的组网算子，从而可更快捷地实现深度学习组网与训练等功能。两者具体异同点可参见下文 [Tensor 与 Numpy 数组相互转换](#newtensor6)。
...
与 Numpy 创建数组方式类似，通过给定 Python 序列（如列表 list、元组 tuple），可使用 [paddle.to_tensor](../../../docs/api/paddle/to_tensor_cn.rst) 创建任意维度的 Tensor。示例如下：
...
<center><img src="https://github.com/PaddlePaddle/docs/blob/develop/docs/guides/beginner/images/Tensor_2.0.png" width="800" ></center>
...
> **说明：**
>
> * 飞桨也支持将 Tensor 转换为 Python 序列数据，可通过 [paddle.tolist](../../../docs/api/paddle/tolist_cn.rst) 实现，飞桨实际的转换处理过程是 **Python 序列 <-> Numpy 数组 <-> Tensor**。
> * 基于给定数据创建 Tensor 时，飞桨是通过拷贝方式创建，与原始数据不共享内存。

### <span id="newtensor2">2.2 指定形状创建</span>

如果要创建一个指定形状的 Tensor，可以使用 [paddle.zeros](../../../docs/api/paddle/zeros_cn.rst)、[paddle.ones](../../../docs/api/paddle/ones_cn.rst)、[paddle.full](../../../docs/api/paddle/full_cn.rst) 实现。
```python
...
如果要在指定区间内创建 Tensor，可以使用[paddle.arange](../../../docs/api/paddle/arange_cn.rst)、 [paddle.linspace](../../../docs/api/paddle/linspace_cn.rst)  实现。
```python
...
>  * **创建一个空 Tensor**，即根据 shape 和 dtype 创建尚未初始化元素值的 Tensor，可通过 [paddle.empty](../../../docs/api/paddle/empty_cn.rst) 实现。
>  * **创建一个与其他 Tensor 具有相同 shape 与 dtype 的 Tensor**，可通过  [paddle.ones_like](../../../docs/api/paddle/ones_like_cn.rst) 、 [paddle.zeros_like](../../../docs/api/paddle/zeros_like_cn.rst) 、 [paddle.full_like](../../../docs/api/paddle/full_like_cn.rst) 、[paddle.empty_like](../../../docs/api/paddle/empty_like_cn.rst) 实现。
>  * **拷贝并创建一个与其他 Tensor 完全相同的 Tensor**，可通过 [paddle.clone](../../../docs/api/paddle/clone_cn.rst) 实现。
>  * **创建一个满足特定分布的 Tensor**，如 [paddle.rand](../../../docs/api/paddle/rand_cn.rst),  [paddle.randn](../../../docs/api/paddle/randn_cn.rst) , [paddle.randint](../../../docs/api/paddle/randint_cn.rst) 等。
>  * **通过设置随机种子创建 Tensor**，可每次生成相同元素值的随机数 Tensor，可通过 [paddle.seed](../../../docs/api/paddle/seed_cn.rst) 和  [paddle.rand](../../../docs/api/paddle/rand_cn.rst) 组合实现。
...
* 对于图像场景，可使用 [paddle.vision.transforms.ToTensor](../../../docs/api/paddle/vision/transforms/Totensor_cn.rst) 直接将 PIL.Image 格式的数据转为 Tensor，使用 [paddle.to_tensor](../../../docs/api/paddle/to_tensor_cn.rst)  将图像的标签（Label，通常是 Python 或 Numpy 格式的数据）转为 Tensor。
* 对于文本场景，需将文本数据解码为数字后，再通过 [paddle.to_tensor](../../../docs/api/paddle/to_tensor_cn.rst) 转为 Tensor。不同文本任务标签形式不一样，有的任务标签也是文本，有的则是数字，均需最终通过 paddle.to_tensor 转为 Tensor。
...
> **说明：**
>
>实际编码时，由于飞桨数据加载的 [paddle.io.DataLoader](../../../docs/api/paddle/io/DataLoader_cn.rst) API 能够将原始 [paddle.io.Dataset](../../../docs/api/paddle/io/Dataset_cn.rst) 定义的数据自动转换为 Tensor，所以可以不做手动转换。具体如下节介绍。

### <span id="newtensor5">2.5 自动创建 Tensor 的功能介绍</span>

除了手动创建 Tensor 外，实际在飞桨框架中有一些 API 封装了 Tensor 创建的操作，从而无需用户手动创建 Tensor。例如 [paddle.io.DataLoader](../../../docs/api/paddle/io/DataLoader_cn.rst) 能够基于原始 Dataset，返回读取 Dataset 数据的迭代器，迭代器返回的数据中的每个元素都是一个 Tensor。另外在一些高层 API，如 [paddle.Model.fit](../../../docs/api/paddle/Model_cn.rst) 、[paddle.Model.predict](../../../docs/api/paddle/Model_cn.rst) ，如果传入的数据不是 Tensor，会自动转为 Tensor 再进行模型训练或推理。
> **说明：**
>
...
形状是  Tensor 的一个重要的基础属性，可以通过  [Tensor.shape](../../../docs/api/paddle/tensor_cn.rst#shape) 查看一个 Tensor 的形状，以下为相关概念：
...
<center><img src="https://github.com/PaddlePaddle/docs/blob/develop/docs/guides/beginner/images/Axis_2.0.png" width="800" ></center>
...
重新设置 Tensor 的 shape 在深度学习任务中比较常见，如一些计算类 API 会对输入数据有特定的形状要求，这时可通过 [paddle.reshape](../../../docs/api/paddle/reshape_cn.rst) 接口来改变 Tensor 的 shape，但并不改变 Tensor 的 size 和其中的元素数据。
...
>  * [paddle.squeeze](../../../docs/api/paddle/squeeze_cn.rst)，可实现 Tensor 的降维操作，即把 Tensor 中尺寸为 1 的维度删除。
>  * [paddle.unsqueeze](../../../docs/api/paddle/unsqueeze_cn.rst)，可实现 Tensor 的升维操作，即向 Tensor 中某个位置插入尺寸为 1 的维度。
>  * [paddle.flatten](../../../docs/api/paddle/flatten_cn.rst)，将 Tensor 的数据在指定的连续维度上展平。
>  * [paddle.transpose](../../../docs/api/paddle/transpose_cn.rst)，对 Tensor 的数据进行重排。

**3.1.3 原位（Inplace）操作和非原位操作的区别**

飞桨框架的 API 有原位（Inplace）操作和非原位操作之分，原位操作即在原 Tensor 上保存操作结果，输出 Tensor 将与输入 Tensor 共享数据，并且没有 Tensor 数据拷贝的过程。非原位操作则不会修改原 Tensor，而是返回一个新的 Tensor。通过 API 名称区分两者，如 [paddle.reshape](../../../docs/api/paddle/reshape_cn.rst) 是非原位操作，[paddle.reshape_](../../../docs/api/paddle/reshape__cn.rst) 是原位操作。
...
Tensor 的数据类型 dtype 可以通过  [Tensor.dtype](../../../docs/api/paddle/tensor_cn.rst#dtype) 查看，支持类型包括：`bool`、`float16`、`float32`、`float64`、`uint8`、`int8`、`int16`、`int32`、`int64`、`complex64`、`complex128`。
...
* 通过给定 Python 序列创建的 Tensor，可直接使用 dtype 参数指定。如果未指定：
    ...
    * 对于 Python 浮点型数据，默认会创建 `float32` 型 Tensor，并且可以通过 [paddle.set_default_dtype](../../../docs/api/paddle/set_default_dtype_cn.rst) 来调整浮点型数据的默认类型。
```python
# 创建 Tensor 时指定 dtype
...
飞桨框架提供了[paddle.cast](../../../docs/api/paddle/cast_cn.rst) 接口来改变 Tensor 的 dtype：
```python
...
初始化 Tensor 时可以通过 [Tensor.place](../../../docs/api/paddle/tensor_cn.rst#place) 来指定其分配的设备位置，可支持的设备位置有：CPU、GPU、固定内存、XPU（Baidu Kunlun）、NPU（Huawei）、MLU（寒武纪）、IPU（Graphcore）等。其中固定内存也称为不可分页内存或锁页内存，其与 GPU 之间具有更高的读写效率，并且支持异步传输，这对网络整体性能会有进一步提升，但其缺点是分配空间过多时可能会降低主机系统的性能，因为其减少了用于存储虚拟内存数据的可分页内存。
> **说明：**
>
> * 当未指定 place 时，Tensor 默认设备位置和安装的飞桨框架版本一致。如安装了 GPU 版本的飞桨，则设备位置默认为 GPU，即 Tensor 的`place` 默认为 [paddle.CUDAPlace](../../../docs/api/paddle/CUDAPlace_cn.rst)。
> * 使用 [paddle.device.set_device](../../../docs/api/paddle/device/set_device_cn.rst) 可设置全局默认的设备位置。Tensor.place 的指定值优先级高于全局默认值。
...
<center><img src="https://github.com/PaddlePaddle/docs/blob/develop/docs/guides/beginner/images/Tensor_broadcast.png" width="800" ></center>
...
如果已有 Numpy 数组，可使用 [paddle.to_tensor](../../../docs/api/paddle/to_tensor_cn.rst) 创建任意维度的 Tensor，创建的 Tensor 与原 Numpy 数组具有相同的形状与数据类型。
```python
...
相对应地，飞桨也支持将 Tensor 转换为 Numpy 数组，可通过 [Tensor.numpy](../../../docs/api/paddle/tensor_cn.rst#numpy) 方法实现。
```python
...
Tensor 作为飞桨框架中重要的数据结构，具有丰富的 API 用以对 Tensor 进行创建、访问、修改、计算等一系列操作，从而满足深度学习任务的需要。更多 Tensor 相关的介绍，请参考 [paddle.Tensor](../../../docs/api/paddle/tensor_cn.rst) API 文档。
