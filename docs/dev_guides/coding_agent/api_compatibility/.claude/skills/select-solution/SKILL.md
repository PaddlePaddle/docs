...
**注意事项**：
- **文档可能滞后或有误**：发现问题时应自行分析，或结合转写配置/Paddle 源码进一步确认
...
1. **优先级规则**：若任意一种信息源判定为"API 完全一致"，则最终结果为"API 完全一致"
2. **多数一致规则**：若多种信息源对同一差异点有一致的判定，采纳该判定
3. **源码优先规则**：当信息源之间判定不一致时，以源码分析结果为准（源码反映真实情况）
